...
POST_NORMAL_SEQUENCE = [
    ...
    {"name": "Episode",           "image": os.path.join(_DIR, "images/buttons/episodes_btn.png")},
...
CHECK_BLANK_LIST          = os.path.join(_DIR, "images/icons/blank_list.png")
...
def run(stop_event=None):
    ...
    while True:
        ...
        # Step 1 — Click coordinates to open grid
        print("\n--- Initial Clicks ---")
        do_initial_clicks()

        # Step 2 — Check if list is empty (no more entries)
        print("\n--- Check List ---")
        if check_state("Blank List", CHECK_BLANK_LIST):
            print("  → No more entries found. Bot finished!")
            break  

        # Step 3 — Check if Normal EP is finished
        print("\n--- Check Normal Episode ---")
        normal_finished = check_state("Normal EP Finished", CHECK_NORMAL_EP_FINISHED)
